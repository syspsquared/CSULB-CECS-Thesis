...
\gradmonth{January}  % month & year of the graduation
\gradyear{2013}  % month & year of the graduation
\department{Department of Computer Engineering and Computer Science} % department
\degree{Master of Science in Computer Science} % CSULB degree
...
\thirdmember{Tracy Bradley Maples, Ph.D.} 
...
%\input{introduction}
\input{intro_to_problem}
\input{intro_to_case_study}
\input{documentation_solution}
\input{student_faculty_help}

\input{printservers}
\input{filesystem_management}
\input{lab_printing}

\input{lab_image}
\input{cloning}

\input{ssh_push_writeup}
\input{unclogging}
\input{local_repository}

\input{specialty_labs}
\input{crossplatform}
\input{conclusion}

%UNEEDED because it was shifted down.
%add a header to the TOC just before making appendix. 
%\addtocontents{toc}{\protect\contentsline{chapterhead}{\null}{Page}{}}
%These are in a different directory. Hence the input command.
\appendix
\input{../appendices/appendix_lab_infrastructure}
\input{../appendices/appendix_lab_installs}
\input{../appendices/appendix_lab_maintenance}
\input{../appendices/appendix_restrict}
\input{../appendices/appendix_special}

%\input{appendix_a} %-Use this to make a thesis office approved TOC, LOF, and LOT
%\input{appendix_b} %-Use this to make a thesis office approved TOC, LOF, and LOT
% \input{detailedDescriptionOfTheCircuit}
% \input{circuitConstruction}
